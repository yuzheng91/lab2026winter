  ...
  },
  {
    ...
    renderCell: (params) => {
      ...
      const fullText =
        ...
        `${r["Input Hit"] || 0}/${r["TotalInput"] || 1} (${(((r["Input Hit"] || 0) / (r["TotalInput"] || 1)) * 100).toFixed(2)}%)`;

      // 2. 利用 '/' 將字串切開，抓出分子與後面的文字
      ...
      return (
        <Typography variant="body2" component="div">
          ...
          <Box
            ...
            onClick={(e) => {
              e.stopPropagation(); // 避免觸發整行 row 的點擊事件
...
const AspectResultPanel = ({ data, methodLabel, onObservedClick }) => {
  ...
  return (
    <Box sx={{ mt: 2, height: 600, width: "100%" }}>
      <DataGrid
        rows={data}
        columns={getColumns(methodLabel, onObservedClick)} // 傳入 callback
        initialState={{
...
const GraphicViewPanel = ({ data, threshold }) => {
  ...
  const cutoffLine =
    threshold === "None" ? 0 : -Math.log10(parseFloat(threshold) || 0.05);
  ...
  const bubbleData = data.map((row, index) => ({
    ...
    name: row["Term ID"]?.startsWith("GO:")
      ? `${row["Term ID"]} (${row["Term Name"]})`
      : row["Term Name"],
    color: row.direction === "Enriched" ? "#ef5350" : "#42a5f5",
  }));
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const displayName = d["Term ID"]?.startsWith("GO:")
        ? `${d["Term ID"]} (${d["Term Name"]})`
        : d["Term Name"];
      return (
        ...
        <ResponsiveContainer>
          ...
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <YAxis
              ...
              dataKey="y"
              name="-log10(P)"
              width={160} // ★ 關鍵：強制鎖定 Y 軸寬度，讓它跟下方的 Bubble Plot 對齊
              label={{
                value: "-log10(P-value)",
                angle: -90,
                position: "insideLeft",
...
export default function WormBase() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const runFpc =
        selectedAspects.F || selectedAspects.P || selectedAspects.C;
  ...
  const handleSample = () => {
    ...
    setInputText(
      ["WBGene00002324", "WBGene00003164", "WBGene00004343"].join("\n"),
    );
  };
  ...
  const displayResult = useMemo(() => {
    ...
    const normalizeRow = (row, direction, isGeneGroup) => {
      ...
      return {
        ...
        "Fold Change": raw_fc,
        log2_fc,
        neg_log10_p, // 確保這兩個數值有塞進去給 Volcano Plot 抓
    ...
    const processList = (list, direction, isGeneGroup) => {
      if (!list) return [];
      return list
        .map((row) => normalizeRow(row, direction, isGeneGroup))
        .filter((row) => row.display_p <= threshVal);
    };

    const mergeData = (aspectData, isGeneGroup) => {
      if (!aspectData) return [];
      const enriched = processList(
        aspectData.enriched,
        "Enriched",
        isGeneGroup,
      );
      const depleted = processList(
        aspectData.depleted,
        "Depleted",
        isGeneGroup,
      );
  ...
  const handleDownload = () => {
    ...
    data.forEach((row) => {
      const obsRatioStr =
        row["Observed Ratio Text"] ||
        `${row["Input Hit"]}/${row["TotalInput"]} (${((row["Input Hit"] / row["TotalInput"]) * 100).toFixed(2)}%)`;
      const expRatioStr =
        row["Expected Ratio Text"] ||
        `${row["Gene Count"]}/${row["TotalInAspect"]} (${((row["Gene Count"] / row["TotalInAspect"]) * 100).toFixed(2)}%)`;
      const featureName = row["Term ID"]?.startsWith("GO:")
        ? `${row["Term ID"]} (${row["Term Name"]})`
        : row["Term Name"];
      ...
      csvRows.push([
        ...
        row["log2_fc"],
        row["neg_log10_p"],
        row["neg_log10_fdr"] || row["neg_log10_p"],
      ]);
    });
    ...
    link.setAttribute(
      "download",
      `${tabNameMap[tabValue] || "Export"}_Enrichment.csv`,
    );
    ...
    <Box sx={{ p: 4, maxWidth: 1400, margin: "0 auto" }}>
            ...
            <Box sx={{ p: 2 }}>
              ...
              >
                <AccordionSummary
                  ...
                  sx={{ bgcolor: styles.accentBlueBg, minHeight: 48 }}
                >
      ...
      {displayResult && (
        <Box sx={{ mt: 5 }}>
          <Paper
            elevation={0}
            sx={{ border: styles.outerBorder, mb: 4, overflow: "hidden" }}
          >
            <Box
              sx={{
            ...
            <Box>
              ...
              >
                ...
                <Box sx={{ flex: 1, p: 2, textAlign: "center" }}>
                  <Typography variant="body2">{listName || "N/A"}</Typography>
                </Box>
              </Box>
              <Box
                sx={{
              ...
              <Box
                sx={{ bgcolor: styles.darkBarBg, p: 1, textAlign: "center" }}
              >
                  ...
                  {ASPECTS.map(
                    (aspect) =>
                      selectedAspects[aspect.key] && (
                        <Button
                          key={aspect.key}
                          onClick={() => handleTabChange(aspect.key)}
                          sx={styles.tabBtnStyle(tabValue === aspect.key)}
                        >
                          {aspect.btnLabel}
                        </Button>
                      ),
                ...
                >
                  {selectedAspects.G && (
                    <Button
                      ...
                      sx={styles.tabBtnStyle(tabValue === "G")}
                    >
          ...
          <Box sx={{ mb: 2, display: "flex", alignItems: "center" }}>
            ...
            <Typography variant="h5">
              Results of the analyzed feature:{" "}
              <b>{getFeatureLabel(tabValue)}</b>
            </Typography>
          </Box>

          <Paper
            elevation={0}
            sx={{ border: styles.outerBorder, mb: 3, bgcolor: "#fbfbfb" }}
          >
            <Box
              sx={{
                p: 1.5,
            ...
            >
              ...
              <Typography variant="body2" fontWeight="bold">
                Specify the correction method and p-value cutoff for multiple
                hypotheses testing
              </Typography>
            </Box>
          ...
          {viewMode === "table" && (
            <Paper elevation={0} sx={{ border: styles.outerBorder, p: 2 }}>
              <Box sx={{ mb: 2, p: 2, bgcolor: "#fafafa", borderRadius: 1 }}>
                <Typography variant="body2">
                  <strong>A:</strong> # of input genes with this term
                </Typography>
                <Typography variant="body2">
              ...
              <AspectResultPanel
                ...
                onObservedClick={handleOpenDetail} // ★ 就是漏了這一行！
              />
            </Paper>
          )}
      ...
      <Dialog
        ...
        PaperProps={{ 
          sx: { 
            bgcolor: "#f5f5f5", 
            maxHeight: "90vh" // ★ 新增：限制視窗最大高度為螢幕的 90%，確保一定會出現滾動條
          } 
        }}
      >
        <DialogTitle
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
              ...
              <Paper variant="outlined" sx={{ overflow: "hidden" }}>
                ...
                <Table size="small">
                  <TableBody>
                    <TableRow>
                      <TableCell sx={{ width: "30%", bgcolor: "#fafafa" }}>
                        Analyzed Term
                      </TableCell>
                      <TableCell>{selectedDetailRow["Term ID"]}</TableCell>
                    </TableRow>
                    <TableRow>
                    ...
                    <TableRow>
                      ...
                      <TableCell>
                        {method} correction, Cutoff = {threshold}
                      </TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
              ...
              <Paper variant="outlined" sx={{ overflow: "hidden" }}>
                ...
                <Box sx={{ p: 2 }}>
                  <Typography variant="body2" mb={0.5}>
                    <b>A:</b> # of input genes
                  </Typography>
                  <Typography variant="body2" mb={0.5}>
                    <b>B:</b> # of input genes that belong to the term [
                    {selectedDetailRow["Term ID"]}]
                  </Typography>
                  <Typography variant="body2" mb={0.5}>
                    <b>C:</b> # of available genes in the genome (—)
                  </Typography>
                  <Typography variant="body2" mb={2}>
                    <b>D:</b> # of genes in the genome that belong to the term [
                    {selectedDetailRow["Term ID"]}]
                  </Typography>
                    ...
                    <Table size="small">
                      <TableHead sx={{ bgcolor: "#eeeeee" }}>
                        <TableRow>
                          ...
                          <TableCell>
                            <b>Observed Ratio (B/A)</b>
                          </TableCell>
                          <TableCell>
                            <b>Expected Ratio (D/C)</b>
                          </TableCell>
                        </TableRow>
                      </TableHead>
                      <TableBody>
                        <TableRow
                          sx={{
                            bgcolor:
                              selectedDetailRow.direction === "Enriched"
                                ? "#ffebee"
                                : "#e3f2fd",
                          }}
                        >
                          ...
                          <TableCell>
                            {Number(selectedDetailRow.display_p).toExponential(
                              4,
                            )}
                          </TableCell>
                          <TableCell>
                            {Number(selectedDetailRow["Fold Change"]).toFixed(
                              2,
                            )}
                          </TableCell>
                          <TableCell>
                            {selectedDetailRow["Observed Ratio Text"] ||
                              `${selectedDetailRow["Input Hit"]}/${selectedDetailRow["TotalInput"]} (${((selectedDetailRow["Input Hit"] / selectedDetailRow["TotalInput"]) * 100).toFixed(4)}%)`}
                          </TableCell>
                          <TableCell>
                            {selectedDetailRow["Expected Ratio Text"] ||
                              `${selectedDetailRow["Gene Count"]}/${selectedDetailRow["TotalInAspect"]} (${((selectedDetailRow["Gene Count"] / selectedDetailRow["TotalInAspect"]) * 100).toFixed(4)}%)`}
                          </TableCell>
                        </TableRow>
                      </TableBody>
                    </Table>
              ...
              <Paper variant="outlined" sx={{ overflow: "hidden" }}>
                ...
                {/* 這裡我們巧妙地複用你原本的 GraphicViewPanel，但只傳入被選中的那筆資料，就會畫出圖上的單顆點 */}
                <Box
                  sx={{
                    pointerEvents:
                      "none" /* 加上這個可以避免單點時 tooltip 一直閃爍干擾 */,
                  }}
                >
                  <GraphicViewPanel
